...
- **`adf tidy` no longer folds compiled vendor output back into `.ai/`** (`#296`) — one marker list was answering two different questions. `adf compile --write` renders the whole ruleset into `CLAUDE.md` behind a `GENERATED by charter adf compile` banner, and that banner was a member of `POINTER_MARKERS`, so `adf tidy` classified the artifact as a thin pointer and treated everything past its preamble — the compiled copy of `.ai/` itself — as content the user had added beyond the pointer. Every run copied the rendered ruleset back into the source it was rendered from, and the pre-commit hook committed the result silently: the reporter's hand-authored 26-line `core.adf` reached 66 lines, with the module index, `METRICS` and `state.adf`'s `STATE` arriving inside `core.adf` `CONSTRAINTS`, and a rule they had deliberately deleted restored to `[load-bearing]` from the stale artifact. The markers are now two lists with a note at the definition explaining why they must not be merged again. `POINTER_MARKERS` answers "is this file a thin pointer?" and excludes the banner; `CHARTER_OWNED_MARKERS` answers "did Charter author this file?" and is the banner plus every pointer marker. The `adf compile --write` overwrite guard (`#295`), `adf migrate`, `bootstrap`, `doctor` and `score` all ask the ownership question and are unchanged — a compile artifact stays overwritable without `--force`, and stays something `migrate` will not ingest. `adf tidy` asks the pointer question, and rather than skipping in silence it now names the file as compiler output and points at `adf compile --write` as the way to regenerate it. The wider vendor-mode questions this exposes — `compile --check` reporting thin pointers as permanently stale, and `adf sync --write` writing an empty lock — are filed separately as `#318` and are not addressed here.
