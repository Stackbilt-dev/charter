...
- **`charter bootstrap` now prints every phase's warnings, not only some of them** (`#307`) — every phase added `step.warnings.length` to the `Bootstrap complete. N warnings.` tally, but phases 6 (`populate`) and 7 (`doctor`) never printed the text, so a default run reported a count with nothing on screen to read and the wording was reachable only under `--format json`. That is the same silence `#304` fixed in phase 2, where it had been hiding advice to run a destructive `bootstrap --force` (`#305`) — so rather than copy that print loop into two more places, recording a phase and showing it are now one operation. Every phase passes through a single `completePhase` helper that appends the step to the result, draws the phase's section, and prints as `Warning: <text>` whatever that section did not already word itself; it is the only code path into `result.steps`, so a phase cannot reach the JSON payload without its warnings reaching the screen, and a future phase 8 cannot reintroduce the gap by omission. Phases that word their own warnings are untouched and still print them once: phase 1's wrapped not-a-git-repo notice, phase 4's dry-run list, and phase 5's `Failed:`/`Hint:`/`Retry manually:` lines. Two further warnings that were recorded but never shown now appear — `Detection failed:` from phase 1's failure path and `Migration failed:` from phase 4's — and the summary tally itself is unchanged.
- **`charter bootstrap` now recognises a Windows absolute `--ai-dir` on a POSIX host** (`#307`) — the check that warns when a git-tracked `.mcp.json` pins a machine-specific `--ai-dir` (`#298`) gated on `path.isAbsolute`, which is the POSIX implementation on Linux and macOS and so reports `false` for `C:\Users\...` and `\\server\share\...`. A `.mcp.json` authored on native Windows is read on a POSIX host every time it reaches CI, a container or a colleague's checkout — precisely the cross-machine sharing the warning exists to catch — and it passed in silence. The value is now tested under `path.win32` as well as the host's own flavour, so drive-letter roots (backslash or forward slash) and UNC roots are detected wherever bootstrap runs; a rooted, driveless `\project\.ai` is absolute under `path.win32` too and is flagged for the same machine-specific reason. Drive-relative `C:project\.ai`, which is absolute under neither flavour, and the repo-relative `.ai` that bootstrap itself writes stay unflagged, and the tracked-ness gate from `#298` is unchanged: an untracked `.mcp.json` remains the documented machine-local escape hatch.
- **`charter doctor` now reports a telemetry log that is already tracked by git** (`#306`) — `#298` stopped newly bootstrapped repos from staging `.charter/telemetry/events.ndjson` by writing `telemetry/` into the generated `<configPath>/.gitignore`, but an ignore rule never untracks a file already in the index. A repo bootstrapped before that change had nothing stopping the log from being staged, and once it is tracked every `git add -A` stages it again — while the documented one-time cleanup only helps people who go looking for it. `doctor` now reads the git index for `<configPath>/telemetry`, names the tracked files, and prints the remedy in the order that actually works: ensure `telemetry/` is ignored *first*, then `git rm -r --cached "<configPath>/telemetry"` — otherwise the next reflexive `git add -A` re-stages the file, since the untrack leaves it on disk so `charter telemetry report` keeps working. The diagnostic is advisory (`INFO`) and **never fails CI**: the condition is state Charter's own bootstrap created rather than anything the user opted into, so reporting it cannot break an existing repo's build on upgrade — the same ruling as the source LOC budget nudge (`#186`). The check honours `--config`, consults only the index so the result is identical on case-sensitive and case-insensitive checkouts, and is withheld under `--adf-only` and outside a git repository.
...
- **`adf populate` no longer invents an ESM extension rule for bundler projects** (`#294`) — the constraint "Use .js extensions for all ESM imports" was emitted whenever any `package.json` carried `"type": "module"`, which is true of every Vite, esbuild and modern Workers project. Under `moduleResolution: "bundler"` extensionless relative imports are the correct convention, so bootstrap was writing a wrong rule into `CONSTRAINTS [load-bearing]` and inviting agents to "fix" a correct codebase. The rule is now gated on TypeScript's `moduleResolution` — emitted for `node16`/`nodenext`, withheld for `bundler`, `node` and anything unrecognised — and a single bundler package vetoes it for the repository. Ambiguity (an unparseable JSONC `tsconfig.json`, a solution-style config, an `extends` chain this resolver does not follow) withholds the rule rather than guessing.
...
- **`charter score` no longer counts directory-tree filenames as broken references** (`#293`) — a fenced tree block expresses the path prefix structurally, through indentation, so `random.ts` and `simulation/random.ts` were reported broken although `src/simulation/random.ts` exists. Grounding now falls back to matching a reference against the repository's real files before classifying it as broken. Two conditions must both hold: the basename is unique, and every path segment the author actually wrote agrees with the real file's trailing segments. The second is checked against the authored text rather than the resolved path, so a reference written inside `.ai/` still resolves, while a stale `src/utils/helper.ts` pointing at a file since moved to `packages/core/lib/helper.ts` stays broken — documentation rot is precisely what this check exists to surface. An ambiguous basename such as `index.ts` under several directories, and a filename that exists nowhere, also stay broken. On this repository the broken count falls from 79 to 48 and Grounding rises from 13/20 to 15/20, while genuinely absent files (`Cargo.toml`, `wrangler.toml`, `schema.sql`, `src/worker.ts`) are all still reported.
...
- **`charter adf compile`** — outbound compiler that renders the modular `.ai/*.adf` source tree to flat vendor agent-config files (`CLAUDE.md`, `AGENTS.md`, `.cursorrules`, `GEMINI.md`). "Babel for agent configs" — ADF is the source of truth, vendor files are build artifacts. This is the reverse of `charter adf migrate`.
  ...
  - `--check` — recompile and diff against files on disk; exit 1 if stale (CI drift gate), 0 if current.
  - `--format json` — machine-readable output with `nextActions` hints.
  - Every generated file begins with a `GENERATED by charter adf compile` banner (HTML comment for `.md` files, `#`-prefixed for `.cursorrules`). The banner is used for overwrite-protection: files without it are treated as hand-authored.
