...
- **`charter score` no longer counts directory-tree filenames as broken references** (`#293`) — a fenced tree block expresses the path prefix structurally, through indentation, so `random.ts` and `simulation/random.ts` were reported broken although `src/simulation/random.ts` exists. Grounding now falls back to basename resolution against the repository's tracked files before classifying a reference as broken. Only a unique, exact-case basename match resolves: an ambiguous basename such as `index.ts` under several directories, and a filename that exists nowhere, both stay broken. The index is built from `git ls-files --exclude-standard`, so dependencies and build output cannot resolve a reference. On this repository the broken count falls from 79 to 47 and Grounding from 13/20 to 15/20, while genuinely absent files (`Cargo.toml`, `wrangler.toml`, `schema.sql`) are still reported.
