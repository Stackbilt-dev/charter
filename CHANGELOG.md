...
- **`charter doctor` now warns when the telemetry log is already tracked by git** (`#306`) — `#298` stopped newly bootstrapped repos from staging `.charter/telemetry/events.ndjson` by writing `telemetry/` into the generated `<configPath>/.gitignore`, but an ignore rule never untracks a file already in the index. Repos bootstrapped before that change keep staging every appended telemetry line on each `git add -A`, and the documented one-time cleanup only helps people who go looking for it. `doctor` now reads the git index for `<configPath>/telemetry`, names the tracked files, and prints the remedy — `git rm -r --cached <configPath>/telemetry`, which leaves the log on disk so `charter telemetry report` keeps working — alongside the `telemetry/` ignore entry that stops it being re-staged. The check honours `--config`, consults only the index so the result is the same on case-sensitive and case-insensitive checkouts, and is withheld outside a git repository and under `--adf-only`, so the existing `doctor --adf-only --ci` gate is unchanged.
