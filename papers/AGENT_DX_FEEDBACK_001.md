...
1. Read the full README (289 lines) — truncated by the tool interface
2. Found `loadLockFile` via ripgrep at line 430 of compiled `adf.js`
3. Dumped the entire 759-line compiled JS — truncated again
...
A flat JSON map of `filename → sha256-prefix(16)`. The agent spent hundreds
...
`SyncStatus`) are internal to the CLI command handler — not exported from
...
The agent's tool interface truncated file reads with `… +759 lines` but
...
| Tool calls | 0 (context already loaded) | 6+ (search → read → truncate → retry) |
