...
`;
  ...
  it('does not feed compiled vendor output back into ADF modules', { timeout: 60000 }, () => {
    const tmp = makeTempDir('hook-compiled');
    writeHookFixtureRepo(tmp);

    const compiled = `<!-- GENERATED by charter adf compile — DO NOT EDIT. Source of truth: .ai/ modules. -->
...
`;
    fs.writeFileSync(path.join(tmp, 'CLAUDE.md'), compiled);
    const coreBefore = fs.readFileSync(path.join(tmp, '.ai/core.adf'), 'utf-8');

    git(tmp, 'add', 'CLAUDE.md');
    git(tmp, 'commit', '-m', 'commit compiled vendor output');

    expect(fs.readFileSync(path.join(tmp, 'CLAUDE.md'), 'utf-8')).toBe(compiled);
    expect(fs.readFileSync(path.join(tmp, '.ai/core.adf'), 'utf-8')).toBe(coreBefore);
  });
