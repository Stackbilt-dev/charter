...
import type { AdfDocument, PatchOperation, MigrationItem, MigrationPlan, TriggerMap } from '@stackbilt/adf';
...
export async function adfMigrateCommand(options: CLIOptions, args: string[]): Promise<number> {
  ...
  // Output
  if (options.format === 'json') {
    const output: Record<string, unknown> = { dryRun, mergeStrategy, sources: results };
    if (audit) output.audit = results.map(r => buildAuditReport(r));
...
interface AuditModuleSummary {
  module: string;
  constraints: number;
  context: number;
  advisory: number;
  total: number;
  potentialMisroutes: string[];
}
...
function buildAuditReport(result: SourceMigrationResult): AuditReport {
  if (!result.plan) return { source: result.source, modules: [] };

  const moduleMap = new Map<string, AuditModuleSummary>();

  for (const item of result.plan.migrateItems) {
    const mod = item.classification.targetModule;
    if (!moduleMap.has(mod)) {
      moduleMap.set(mod, { module: mod, constraints: 0, context: 0, advisory: 0, total: 0, potentialMisroutes: [] });
    }
    const entry = moduleMap.get(mod)!;
    entry.total++;
    if (item.classification.targetSection === 'CONSTRAINTS') entry.constraints++;
    if (item.classification.targetSection === 'CONTEXT') entry.context++;
    if (item.classification.targetSection === 'ADVISORY') entry.advisory++;

    // Flag potential misroutes: routed to core.adf but has non-zero scores for specialized modules
    if (mod === 'core.adf' && item.classification.routingTrace) {
      const scores = item.classification.routingTrace.candidateScores;
      const betterModule = Object.entries(scores)
        .filter(([m, s]) => m !== 'core.adf' && s > 0)
        .sort(([, a], [, b]) => (b as number) - (a as number))[0];
      if (betterModule) {
        const preview = item.element.content.length > 50
          ? item.element.content.slice(0, 47) + '...'
          : item.element.content;
        entry.potentialMisroutes.push(`"${preview}" (score: ${betterModule[0]}=${betterModule[1]})`);
      }
    }
  }

  return { source: result.source, modules: [...moduleMap.values()] };
}

function printAuditReport(result: SourceMigrationResult): void {
  if (!result.plan || result.skipped) return;

  const report = buildAuditReport(result);
  console.log(`  Audit: ${report.source}`);

  if (report.modules.length === 0) {
    console.log('    No items migrated.');
    return;
  }

  for (const mod of report.modules) {
    const parts: string[] = [];
    if (mod.constraints > 0) parts.push(`${mod.constraints} constraints`);
    if (mod.context > 0) parts.push(`${mod.context} context`);
    if (mod.advisory > 0) parts.push(`${mod.advisory} advisory`);
    console.log(`    ${mod.module}: ${parts.join(', ')} (${mod.total} total)`);
    for (const misroute of mod.potentialMisroutes) {
      console.log(`      [?] possible misroute: ${misroute}`);
    }
  }
  console.log('');
}
