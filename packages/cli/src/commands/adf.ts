...
/**
 * Typed data access and ontology enforcement policy (charter#69).
 *
 * This scaffold codifies the cross-repo policy for how services reference
 * business concepts (tenant, user, subscription, quota, etc.) — derived from
 * the canonical data registry at Stackbilt-dev/stackbilt_llc/policies/data-registry.yaml.
 *
 * Six sensitivity tiers, disambiguation protocol, and RPC boundary rules.
 * Designed to be loaded on-demand when code touches data access or ontology
 * keywords, and consumed by charter validate / codebeast DATA_AUTHORITY /
 * AEGIS disambiguation firewall.
 */
 ...
 *
 * Modules added here should also be referenced from this file's documentation
 * and — if they have canonical trigger keywords — the adfCreate command will
 * use NAMED_MODULE_DEFAULT_TRIGGERS when no --triggers flag is provided.
 */
