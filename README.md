...
Keep `--ai-dir` repo-relative — `.mcp.json` is committed, so an absolute path breaks it for every other clone. If your MCP client spawns the server outside the project root, set the client's `cwd` or pass an absolute path and keep it untracked. See [CLI reference](docs/cli-reference.md#wiring-in-mcpjson).
