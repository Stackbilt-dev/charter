...
Charter appends a line to `<configPath>/telemetry/events.ndjson` on every run.
Repos bootstrapped before that path was added to the generated
`<configPath>/.gitignore` have it committed, so every `git add -A` stages the
log again — an ignore entry alone never untracks a file already in the index.
